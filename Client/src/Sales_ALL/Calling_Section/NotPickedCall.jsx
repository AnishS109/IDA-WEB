    ...
    const fetchNotPickedCallData = async () => {
      ...
      try {
        ...
        if (filteredData.length > 0) {
          setNotPickedCallData(filteredData);
        } else {
          setNotPickedCallData([]);  // Set an empty array if no data
        }
      } catch (error) {
        ...
        <>
          {notPickedCallData.length > 0 ? (
            <Table>
              ...
              <TableBody>
                {/* Map over notPickedCallData and display items with the "Rejected Call" category */}
